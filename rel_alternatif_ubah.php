...
<?php
$rows = $db->get_results("SELECT ra.kode_rel_alternatif, k.kode_kriteria, k.nama_kriteria, ra.nilai_alternatif FROM tb_rel_alternatif ra INNER JOIN tb_kriteria k ON k.kode_kriteria=ra.kode_kriteria  WHERE kode_alternatif='$_GET[ID]' ORDER BY kode_kriteria");
foreach($rows as $row):?>
<div class="form-group">
    <label><?=$row->nama_kriteria?></label>
    <input type="text" class="form-control" name="ID-<?=$row->kode_rel_alternatif?>" value="<?= $row->nilai_alternatif ?? 0 ?>">    
    <!-- <select class="form-control" name="ID-<?=$row->kode_rel_alternatif?>"><?=get_crips_option($row->kode_kriteria, $row->kode_nilai_kriteria)?></select> -->
</div>
<?php endforeach?>
